...
export function ExportPdfNegotiationButton({ proposal, conditions, targetPrice, standardFlow, projetoNome, logoUrl }: Props) {
    const [isGenerating, setIsGenerating] = useState(false)

    const handleExport = async () => {
        if (conditions.length === 0) return toast.error("Não há condições montadas na proposta para gerar o PDF.")
        ...
        try {
            ...
            // Criamos um tipo local rápido só para o TypeScript reconhecer a origemDescricao
            ...
            const valorTabela = proposal.valorTabelaOriginal || 0

            const nomeDoProjeto = projetoNome || pSafe.projeto?.nome || "Projeto"
            const finalLogoUrl = logoUrl || pSafe.projeto?.logoUrl || null
            ...
            doc.text("Proposta Comercial", textStartX, finalY)
            ...
            autoTable(doc, {
                ...
                body: [
                    ...
                    [{ content: 'Origem:', styles: { fontStyle: 'bold' } }, leadOrigem, { content: 'Valor Tabela:', styles: { fontStyle: 'bold' } }, fmtCurrency(valorTabela)],
                    [{ content: 'Descrição:', styles: { fontStyle: 'bold' } }, leadOrigemDescricao, { content: 'Valor Proposto:', styles: { fontStyle: 'bold' } }, fmtCurrency(targetPrice)]
                ],
                theme: 'plain',
            ...
            const rawFileName = `PROPOSTA_${nomeDoProjeto}_UN${numUnidade}_${leadNome}`
