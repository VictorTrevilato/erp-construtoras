...
import { Building2, ArrowRight, Layers, DoorOpen, Car, MapPin } from "lucide-react"
...
export function CommercialProjectsList({ projects }: { projects: ProjectSummary[] }) {
        ...
        <Table>
          <TableHeader>
            <TableRow className="bg-muted/40">
              ...
              {/* [NOVO] Coluna Tipo antes de Status */}
            ...
            {projects.length === 0 ? (
              <TableRow>
                <TableCell colSpan={7} className="text-center py-10 text-muted-foreground">
                  Nenhum projeto encontrado. Cadastre um projeto no módulo de Engenharia primeiro.
                </TableCell>
              </TableRow>
            ) : (
                ...
                <TableRow key={project.id} className="hover:bg-slate-50 transition-colors">
                  ...
                  <TableCell className="text-center">
                    <div className="flex items-center justify-center gap-1 text-muted-foreground">
                       ...
                       <span className="font-medium text-foreground">{project.totalVagas}</span>
                    </div>
                  </TableCell>

                  <TableCell className="text-right">
                    <Button asChild size="sm" variant="default" className="gap-2">
                      <Link href={`/app/comercial/unidades/${project.id}`}>
                        Acessar Estoque <ArrowRight className="w-4 h-4" />
                      </Link>
                    </Button>
                  </TableCell>
                </TableRow>
